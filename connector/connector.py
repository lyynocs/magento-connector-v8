...
class ConnectorUnit(object):
    ...
    def __init__(self, environ):
        ...
        :param environ: current environ (backend, session, ...)
        :type environ: :py:class:`connector.connector.environ`
        """
        super(ConnectorUnit, self).__init__()
        self.environ = environ
        self.backend = self.environ.backend
        self.backend_record = self.environ.backend_record
        self.session = self.environ.session
        self.model = self.session.pool.get(environ.model_name)
    ...
    def get_connector_unit_for_model(self, connector_unit_class, model=None):
        ...
        :py:class:`~connector.connector.environ`,
        ...
        if model is None:
            ...
            env = Environ(self.backend_record,
                              self.session,
                              model)
...
class Environ(object):
    """ environ used by the different units for the synchronization.
    ...
    def set_lang(self, code):
        """ Change the working language in the environ.
